...
class BIWGAN(object):
    """ Online Variational Wasserstein BIGAN with consistent sampling.
    ...
    def __init__(self, sess, x, input_size, batch_size, latent_size,
                 encoder, decoder, discriminator, is_training, discrete_size, activation=tf.nn.elu,
                 reconstr_loss_type="binary_cross_entropy", learning_rate=1e-4,
                 submodel=0, total_true_models=0, vae_tm1=None,
                 p_x_given_z_func=distributions.Bernoulli, critics=5,
    ...
    def get_name(self):
        if self.submodel == 0:
            ...
            full_hash_str = full_hash_str.strip().lower().replace('[', '')  \
                                                         ...
                                                         .replace(')', '') \
                                                         .replace('\'', '')
            return 'biwgan%d_' % self.submodel + full_hash_str
        else:
    ...
    def _create_constraints(self):
        ...
        if hasattr(self, 'xhat_tm1'):
            ...
            self.q_z_s_given_x_t, self.q_z_t_given_x_t \
                = BIWGAN.zero_pad_smaller_cat(self.q_z_s_given_x_t,
    ...
    def encoder(self, X, rnd_sample=None, reuse=False, hard=False):
        with tf.variable_scope(self.get_name() + "/encoder", reuse=reuse):
            ...
            return BIWGAN.reparameterize(encoded, self.num_discrete,
                                      self.tau, hard=hard,
                                      rnd_sample=rnd_sample)
    ...
    def generator(self, Z, reuse=False):
        with tf.variable_scope(self.get_name() + "/generator", reuse=reuse):
            ...
            return tf.tanh(forward(Z, self.decoder_model))

    def discriminator(self, Z, X, reuse=False):
        with tf.variable_scope(self.get_name() + "/discriminator", reuse=reuse):
            batch_size = shp(X)[0]
            x_flat = tf.reshape(X, [batch_size, -1])
            logits = forward(tf.concat([Z, x_flat], 1),
                             self.discriminator_model)
            logits_batch_size = shp(logits)[0]
            logits_flat = tf.reshape(logits, [logits_batch_size, -1])
            return slim.fully_connected(logits_flat, 1,
                                        activation_fn=None,
                                        scope='output')
    ...
    def generate_at_least(self, vae_tm1, batch_size):
        ...
        return [z, z_cat, p_x_given_z_tm1]
    ...
    def _create_network(self, num_test_memories=10):
        ...
        self.z_prior = BIWGAN.sample_prior(self.num_discrete,
                                           self.latent_size,
                                           self.num_current_data)
        self.fake_data = self.generator(self.z_prior)
        #print 'z_real = ', self.z.get_shape().as_list()

        self.disc_real = self.discriminator(self.z, self.x_augmented)
        self.disc_fake = self.discriminator(self.z_prior, self.fake_data, reuse=True)
        print 'disc_real = ', shp(self.disc_real), ' | disc_fake = ', shp(self.disc_fake)
        # self.x_reconstr_mean_activ = tf.nn.sigmoid(self.x_reconstr_mean)

    @staticmethod
    def sample_prior(num_discrete, latent_size, batch_size):
        z_cat = generate_random_categorical(num_discrete, batch_size)
        z_normal = tf.random_normal([batch_size, latent_size])
        return tf.concat([z_normal, z_cat], axis=1)
    ...
    def wgan_loss(self, real_data, real_z, consistency_kl):
        # Standard WGAN loss w/ consistency
        gen_cost = -tf.reduce_mean(self.disc_fake + tf.expand_dims(consistency_kl, axis=1),
                                   name="gen_cost")
        disc_cost = tf.reduce_mean(self.disc_fake) - tf.reduce_mean(self.disc_real)

        print 'df = ',shp(self.disc_fake), ' | dr = ', shp(self.disc_real), \
            'discw = ', shp(disc_cost), ' | genw = ', shp(gen_cost)

        # Gradient penalty
        alpha_x = tf.random_uniform(
            shape=[shp(real_data)[0], 1, 1, 1],
            minval=0.,
            maxval=1.
        )
        alpha_z = tf.random_uniform(
            shape=[shp(self.z)[0], 1],
            minval=0.,
            maxval=1.
        )
        differences = [self.fake_data - real_data,
                       self.z_prior - self.z]
        print 'ax = ', shp(alpha_x), ' | diff = ', shp(differences[0])
        interpolates = [real_data + (alpha_x*differences[0]),
                        real_z + (alpha_z*differences[1])]

        gradients = tf.gradients(self.discriminator(interpolates[1],
                                                    interpolates[0],
                                                    reuse=True),
                                 interpolates)#[0]
        slopes = tf.sqrt(tf.reduce_sum(tf.square(tf.reshape(gradients[0],
                                                            [shp(gradients[0])[0], -1])),
                                       reduction_indices=[1]))
        slopes += tf.sqrt(tf.reduce_sum(tf.square(gradients[1]), reduction_indices=[1]))
        gradient_penalty = tf.reduce_mean((slopes-1.)**2)
        disc_cost += 10*gradient_penalty  # XXX: parameterize
        disc_cost = tf.identity(disc_cost, name="disc_cost")

        # mutual_info_regularizer \
        #     = BIWGAN.mutual_information_bernouilli_cat(self.z_pre_gumbel)
        # gen_cost -= self.mutual_info_reg * mutual_info_regularizer
    ...
    def _create_loss_optimizer(self):
        ...
        self.gen_cost, self.disc_cost \
            = self.wgan_loss(self.x_augmented, self.z, self.kl_consistency)

        # construct our optimizer
        #with tf.control_dependencies([self.p_x_given_z_logits]):
        gen_params = [v for v in tf.trainable_variables()
                      if v.name.startswith(self.get_name())
                      and "generator" in v.name
                      or "encoder" in v.name]
        disc_params = [v for v in tf.trainable_variables()
                       if v.name.startswith(self.get_name())
                       and "discriminator" in v.name]
        print '#gen_params = ', len(gen_params), \
            ' | #disc_params = ', len(disc_params)

        with tf.variable_scope(self.get_name() + "/loss_optimizer"):
            self.gen_train_op, self.disc_train_op = self._create_optimizer(gen_params, self.gen_cost,
                                                                           disc_params, self.disc_cost,
                                                                           self.learning_rate)

    def _create_optimizer(self, gen_params, gen_cost, disc_params, disc_cost, lr):
        print 'there are %d trainable vars in cost %s + %s\n' \
            % (len(gen_params) + len(disc_params),
               gen_cost.name, disc_cost.name)
        print 'g0 = %s' % (len(tf.gradients(disc_cost, disc_params)))
        grads = tf.concat([tf.gradients(gen_cost, gen_params),
                           tf.gradients(disc_cost, disc_params)], axis=0)
    ...
    def partial_fit(self, inputs, iteration_print=10,
        ...
        try:
            assert len(inputs) == self.critics
            ...
            # run the generator always
            gen_ops = [self.gen_train_op, self.iteration_gpu_op, self.gen_cost]
            _, _, gen_cost\
                = self.sess.run(gen_ops,
                                feed_dict={self.is_training: True,
                                           self.tau: self.tau_host})

            # run the critic 5 times for example
            disc_ops = [self.disc_train_op, self.disc_cost]
            for i in xrange(self.critics):
                feed_dict = {self.x: inputs[i],
                             self.is_training: True,
                             self.tau: self.tau_host}

                if self.iteration % iteration_save_imgs == 0 and i == (self.critics - 1):
                    disc_ops += [self.image_summaries]
                    _, disc_cost, summary_img \
                        = self.sess.run(disc_ops,
                                        feed_dict=feed_dict)
                    self.summary_writer.add_summary(summary_img, self.iteration)
                elif self.iteration % iteration_print == 0 and i == (self.critics - 1):
                    disc_ops += [self.summaries]
                    _, disc_cost, summaries \
                        = self.sess.run(disc_ops,
                                        feed_dict=feed_dict)
                    self.summary_writer.add_summary(summaries, self.iteration)
                else:
                    _, disc_cost = self.sess.run(disc_ops,
                                                 feed_dict=feed_dict)
    ...
    def build_new_encoder_decoder_pair(self, num_new_classes=1):
        ...
        if self.encoder_model.layer_type is not 'cnn':
            ...
            decoder = DenseEncoder(self.sess, self.input_size,
                                   ...
                                   use_bn=self.decoder_model.use_bn)
            discriminator = DenseEncoder(self.sess, 1,
                                         self.is_training,
                                         scope="discriminator",
                                         sizes=layer_sizes,
                                         use_ln=self.discriminator_model.use_ln,
                                         use_bn=self.discriminator_model.use_bn)
        else:
            ...
            discriminator = CNNEncoder(self.sess, 1,
                                       self.is_training,
                                       scope="discriminator",
                                       use_ln=self.discriminator_model.use_ln,
                                       use_bn=self.discriminator_model.use_bn)
    ...
    def fork(self, num_new_class=1):
        ...
        '''
        encoder, decoder, discriminator = self.build_new_encoder_decoder_pair(num_new_class)
        print 'encoder = ', encoder.get_info()
        print 'decoder = ', decoder.get_info()
        print 'discriminator = ', discriminator.get_info()

        vae_tp1 = BIWGAN(self.sess, self.x,
                         input_size=self.input_size,
    ...
    def generate(self, z=None):
        ...
        # Note: This maps to mean of distribution, we could alternatively
        # sample from Gaussian distribution
        return self.sess.run(self.fake_data,
                             feed_dict={self.z: z,
                                        self.tau: self.tau_host,
                                        self.is_training: False})
    ...
    def train(self, source, batch_size, training_epochs=10, display_step=5):
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(total_batch):
                ...
                gen_cost, disc_cost = self.partial_fit(batch_xs)
                # Compute average loss
                avg_gen_cost += gen_cost / n_samples * batch_size
                avg_disc_cost += disc_cost / n_samples * batch_size

            # Display logs per epoch step
            if epoch % display_step == 0:
                print "[Epoch:", '%04d]' % (epoch+1), \
                    "gen cost = ", "{:.4f} | ".format(gen_cost), \
                    "avg gen cost = ", "{:.4f} | ".format(avg_gen_cost), \
                    "disc cost = ", "{:.4f} | ".format(disc_cost), \
                    "avg disc cost = ", "{:.4f} | ".format(avg_disc_cost), \
