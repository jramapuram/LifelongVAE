...
# An object that filters the classes of svhn
class SVHN_Class(object):
    def __init__(self, class_number, svhn):
        self.input_size = len(svhn.train.images[0])
        ...
        self.classes = self.filter_classes(svhn, self.blacklist)

    @staticmethod
    def filter_classes(svhn, blacklist):
        classes = deepcopy(svhn)
    ...
    def get_validation_batch_iter(self, batch_size):
        images, labels = self.classes.validation.next_batch(batch_size)
        #images, labels = self._augment(images, labels)
        return np.array(images), np.array(labels)
...
class SVHN(object):
    def __init__(self, one_hot):
        train_data, train_labels,\
            valid_data, valid_labels,\
            test_data, test_labels = generate_cropped_files()
        #generate_full_files()

        #XXX: de-one-hotify for Dataset class
