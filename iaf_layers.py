...
def linear(name, x, num_units, init_scale=1., init=False):
    with tf.variable_scope(name):
        if init:
            ...
            return tf.reshape(scale_init, [1, num_units]) \
...
def conv2d(name, x, num_filters, filter_size=(3, 3), stride=(1, 1), pad="SAME",
    ...
    with tf.variable_scope(name):
        if init:
            # data based initialization of parameters
            v = tf.get_variable("V", filter_shape, dtype,
                                tf.random_normal_initializer(0, 0.05,
                                                             dtype=dtype))
            ...
            x_init = tf.nn.conv2d(x, v_norm, stride_shape,
                                  pad, data_format="NCHW")
            m_init, v_init = tf.nn.moments(x_init, [0, 2, 3])
            scale_init = init_scale / tf.sqrt(v_init + 1e-10)
            _ = tf.get_variable("g", initializer=tf.log(scale_init) / 3.0)
            _ = tf.get_variable("b", initializer=-m_init * scale_init)
            return tf.reshape(scale_init, [1, -1, 1, 1]) \
                * (x_init - tf.reshape(m_init, [1, -1, 1, 1]))
        else:
            ...
            # use weight normalization (Salimans & Kingma, 2016)
            w = tf.reshape(tf.exp(g), [1, 1, 1, num_filters]) \
                * tf.nn.l2_normalize(v, [0, 1, 2])

            # calculate convolutional layer output
            b = tf.reshape(b, [1, -1, 1, 1])
            return tf.nn.conv2d(x, w, stride_shape, pad, data_format="NCHW") \
                + b
...
def my_deconv2d(x, filters, strides):
    ...
    output_shape = [int(input_shape[0]), int(filters.get_shape()[2]),
                    int(input_shape[2] * strides[2]),
                    int(input_shape[2] * strides[3])]
    ...
    x = tf.nn.conv2d_transpose(x, filters, output_shape=output_shape,
...
@add_arg_scope
def deconv2d(name, x, num_filters, filter_size=(3, 3), stride=(2, 2),
             pad="SAME", init_scale=0.1, init=False,
             mask=None, dtype=tf.float32, **_):
    stride_shape = [1, 1, stride[0], stride[1]]
    filter_shape = [filter_size[0], filter_size[1],
    ...
    with tf.variable_scope(name):
        if init:
            ...
            # use weight normalization (Salimans & Kingma, 2016)
            w = tf.reshape(tf.exp(g), [1, 1, num_filters, 1]) \
                * tf.nn.l2_normalize(v, [0, 1, 2])
...
def ar_conv2d(name, x, num_filters, filter_size=(3, 3), stride=(1, 1),
    ...
    with arg_scope([conv2d]):
        return conv2d(name, x, num_filters, filter_size, stride,
                      pad, init_scale, mask=mask)
    ...
    with tf.variable_scope(name), arg_scope([ar_conv2d]):
        for i, size in enumerate(n_h):
            ...
            if i == 0:
                x += context
            x = nl(x)
        return [ar_conv2d("layer_out_%d" % i, x, size, zerodiagonal=True)
